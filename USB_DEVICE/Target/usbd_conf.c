...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  hpcd_USB_OTG_FS.Init.dev_endpoints = 4;//6;
  hpcd_USB_OTG_FS.Init.speed = PCD_SPEED_FULL;
    hpcd_USB_OTG_FS.Init.ep0_mps= USB_MAX_EP0_SIZE;

  hpcd_USB_OTG_FS.Init.dma_enable = DISABLE;
  hpcd_USB_OTG_FS.Init.phy_itface = PCD_PHY_EMBEDDED;
  hpcd_USB_OTG_FS.Init.Sof_enable = 0;//ENABLE;
  hpcd_USB_OTG_FS.Init.low_power_enable = DISABLE;
  hpcd_USB_OTG_FS.Init.lpm_enable = DISABLE;
  hpcd_USB_OTG_FS.Init.vbus_sensing_enable = 0;//ENABLE;
  ...
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x20);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, 0x20);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 2, 0x20);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 3, 0x20);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 4, 0x20);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 5, 0x20);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 6, 0x20);
